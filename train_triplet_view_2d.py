...
from dataset.Dataset_Loader import get_2d_loader_view
...
def main(args):
    ...
    if args.backbone == 'vgg11':
        net_shape = ngvnn.Net_Whole(pretraining=args.pretraining).cuda()
        net_sketch = ngvnn.Net_Whole(pretraining=args.pretraining).cuda()
    elif args.backbone == 'resnet50':
        net_shape = ngvnn.Net_Whole_resnet().cuda()
        net_sketch = ngvnn.Net_Whole_resnet().cuda()
    # shape_optim = torch.optim.SGD(net_p.parameters(), lr=args.lr, momentum=0.9, weight_decay=args.weight_decay)
    base_param_ids = set(map(id, net_shape.features.parameters()))
    new_params = [p for p in net_shape.parameters() if id(p) not in base_param_ids]
    param_groups = [
        {'params': net_shape.features.parameters(), 'lr_mult': 0.1},
        {'params': new_params, 'lr_mult': 1.0}]
    shape_optim = torch.optim.SGD(param_groups, lr=args.lr, momentum=0.9, weight_decay=args.weight_decay)

    base_param_ids = set(map(id, net_sketch.features.parameters()))
    new_params = [p for p in net_sketch.parameters() if id(p) not in base_param_ids]
    param_groups = [
        {'params': net_sketch.features.parameters(), 'lr_mult': 0.1},
        {'params': new_params, 'lr_mult': 1.0}]
    ...
    if args.resume_checkpoint:
        ...
        if not os.path.exists(resume_checkpoint):
            resume_checkpoint = Path(os.path.join(args.save_dir, "runs", args.save_name, 'checkpoints',  'checkpoint-latest.pt'))
            if not os.path.exists(resume_checkpoint):
...
def train(crt_tl, sketch_dataloader, shape_dataloader, net_shape, net_sketch, shape_optim, sketch_optim, writer, epoch, args, logger):
    ...
    for bidx, (sketches_data, shapes_data) in enumerate(zip(sketch_dataloader, shape_dataloader)):
        ...
        shapes = shapes.view(shapes.size(0), shapes.size(2), shapes.size(3), shapes.size(4))
