...
from dataset.Dataset_Loader import get_dataloader, get_dataloader_aug
...
def main(args):
    ...
    from dataset.Dataset_Loader import get_dataloader_shape
    train_shape_loader, train_sketch_loader, val_shape_loader, val_sketch_loader = get_dataloader_shape(args)
    if args.train:
        best_epoch = -1
        '''TRANING'''
        ...
        for epoch in range(start_epoch, args.epochs+1):
            ...
            if epoch % args.save_freq == 0:
                ...
                writer.add_scalar('val/val_feature_z', top1, epoch + 1) # mAP_feat_norm
        ...
        cur_metric = validate(logger, val_sketch_loader, val_shape_loader, model, save=True, save_dir=log_dir, batch_size=args.batch_size)
...
def train(sketch_dataloader, shape_dataloader, model, optimizer, writer, epoch, args, logger):
    ...
    for bidx, (sketches, shapes) in enumerate(zip(sketch_dataloader, shape_dataloader)):
        ...
        if step % args.log_freq == 0:
            ...
            start_time = time.time()
            tl_loss, loss = out['tl_loss'], out['loss']
            if writer is not None:
            ...
            log_string(
                "Epoch %d Batch [%2d/%2d] Time [%3.2fs]  Triplet %2.5f loss %2.5f"
                % (epoch, bidx, len(sketch_dataloader), duration, tl_loss, loss), logger)
...
def validate(logger, sketch_dataloader, shape_dataloader, model, save=True, save_dir='',batch_size=0):
    sketch_features = []
    shape_features = []
    shape_point = []
    sketch_point = []
    ...
    with torch.no_grad():
        for i, data in enumerate(sketch_dataloader):
            ...
            sketch_point.append(sketch_points.data.cpu())

        for i, data in enumerate(shape_dataloader):
            shape_points = data[0].cuda()
            shape_z = model.extract_feature(shape_points)
            shape_features.append(shape_z.data.cpu())
            shape_point.append(shape_points.data.cpu())


    shape_features = torch.cat(shape_features, 0).numpy()
    sketch_features = torch.cat(sketch_features, 0).numpy()

    shape_point = torch.cat(shape_point, 0).numpy()
    sketch_point = torch.cat(sketch_point, 0).numpy()

    d_feat_z = compute_distance(sketch_features.copy(), shape_features.copy(), l2=True)
    acc_at_k_feat_z = compute_acc_at_k(d_feat_z)

    if save:
        np.save(os.path.join(save_dir, 'shape_point_{}.npy'.format(batch_size)), shape_point)
        np.save(os.path.join(save_dir, 'sketch_point_{}.npy'.format(batch_size)), sketch_point)
        ...
        np.save(os.path.join(save_dir, 'd_feat_{}.npy'.format(batch_size)), d_feat_z)
...
if __name__ == '__main__':
    ...
    if args.windows:
        ...
        args = parser.parse_args(args=[
            # '--debug',
                                       '--train',
            '--list_file', "network/{}.txt",
                                       '--save_name', 'naive_3dv_shape_shape',
                                       '--lr', '2e-3',
